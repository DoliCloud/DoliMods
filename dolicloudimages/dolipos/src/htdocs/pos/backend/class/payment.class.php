...
/**
 *	\file       htdocs/pos/backend/class/paiement.class.php
 *	\ingroup    ticket
...
/**     \class      Paiement
 *		\brief      Classe permettant la gestion des paiements des tickets clients
 */
class Payment extends CommonObject
{
	...
	// fk_paiement dans llx_paiement_ticket est le rowid du paiement
		...
		if ($resql)
		{
			...
			foreach ($this->amounts as $key => $amount)
			{
				...
				if (is_numeric($amount) && $amount <> 0)
				{
					...
					$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'pos_paiement_ticket (fk_ticket, fk_paiement, amount)';
					$sql .= ' VALUES ('.$ticketid.', '. $this->id.', \''.$amount.'\')';

					dol_syslog(get_class($this).'::Create Amount line '.$key.' insert paiement_ticket sql='.$sql);
					$resql=$this->db->query($sql);
					if ($resql)
					{
					...
					else
					{
						...
						dol_syslog(get_class($this).'::Create insert paiement_ticket error='.$this->error, LOG_ERR);
						$error++;
					}
				}
				else
                    ...
                    foreach ($this->amounts as $key => $value)  // We should have always same third party but we loop in case of.
                    {
                        if ($mode == 'payment')
                        {
                            ...
                            if (! in_array($socid,$linkaddedforthirdparty)) // Not yet done for this thirdparty
                            {
                                ...
                                DOL_URL_ROOT.'/comm/fiche.php?socid=', $thirdparty->nom, 'company');
	...
	/**
	 *      \brief      Retourne la liste des tickets sur lesquels porte le paiement
	 *      \param      filter          Critere de filtre
	 *      \return     array           Tableau des id de tickets
	 */
	function getBillsArray($filter='')
	{
		$sql = 'SELECT fk_ticket';
		$sql.= ' FROM '.MAIN_DB_PREFIX.'paiement_ticket as pf, '.MAIN_DB_PREFIX.'ticket as f';
		$sql.= ' WHERE pf.fk_ticket = f.rowid AND fk_paiement = '.$this->id;
	...
	/**
	 *    	\brief      Retourne le libelle du statut d'une ticket (brouillon, validee, abandonnee, payee)
