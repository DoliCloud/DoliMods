...
else $db = @mysql_connect($dbhost, $dbuser, $dbpass) or showError("Tracker error: can't connect to database. Contact the webmaster.");
	...
	if (substr($_SERVER["PATH_INFO"], -7) == '/scrape') {
		if ($scrape == true) {
			...
			if (isset($_GET["info_hash"])) {
				...
				else showError("Invalid info hash value.");
				$usehash = true;
			}
			if ($usehash)
			$query = mysql_query("SELECT info_hash, filename FROM ".$prefix."namemap WHERE info_hash=\"$info_hash\"");
			else $query = mysql_query("SELECT info_hash, filename FROM ".$prefix."namemap");
			$namemap = array();
			...
			else $query = mysql_query("SELECT info_hash, seeds, leechers, finished FROM ".$prefix."summary ORDER BY info_hash") or showError("Database error. Cannot complete request.");
...
if ($event == '') {
	...
	else $peers = array("size" => 0);

	sendPeerList($peers);
} elseif ($event == "completed") { // now the same as an empty string
	verifyTorrent($info_hash) or evilReject($ip, $peer_id, $port);
	$peer_exists = getPeerInfo($peer_id, $info_hash);
	...
	sendPeerList($peers);
} else showError("Invalid event= from client.");
