...
/**
 *      \file       htdocs/zipautofillfr/fixcase.php
 *		\ingroup    zipautofillfr
 *      \brief      This script fix the case of town into database
 */
...
// Update column town_up if empty
	...
	if ($num)
	{
		while ($i < $num)
		{
			...
			if ($obj)
			{
				...
				print $obj->rowid.' '.$obj->town." -> ".$newtown."\n";

				$sql = "UPDATE ".MAIN_DB_PREFIX."c_ziptown SET town = '".$db->escape($newtown)."' WHERE rowid = ".((int) $obj->rowid);
				$db->query($sql);
			}
			$i++;
		}
...
if (!$error) {
	if ($argv[1] === 'confirm') {
		...
		print 'Test mode, modification are canceled'."\n";
		$db->rollback();
	}
	print '--- end ok'."\n";
} else {
