...
/*
 ...
 $Id: index.php,v 1.1 2011/08/01 19:22:21 eldy Exp $
...
if ($RCMAIL->task == 'login' && $RCMAIL->action == 'login') {
	...
	} elseif ($_SESSION['temp'] && !$auth['abort'] &&
...
elseif ($RCMAIL->task != 'login' && $_SESSION['user_id'] && $RCMAIL->action != 'send') {
...
elseif (!empty($_POST) && !$request_check_whitelist[$RCMAIL->action] && !$RCMAIL->check_request()) {
...
elseif ($RCMAIL->action == 'save-pref') {
...
while ($redirects < 5) {
	...
	if ($RCMAIL->plugins->is_plugin_task($RCMAIL->task)) {
		...
		break;
	} elseif (preg_match('/^plugin\./', $RCMAIL->action)) {
		$RCMAIL->plugins->exec_action($RCMAIL->action);
		break;
	}
	// try to include the step file
	elseif (is_file($incfile = 'program/steps/'.$RCMAIL->task.'/'.$stepfile)) {
		include $incfile;
		$redirects++;
	} else {
		break;
