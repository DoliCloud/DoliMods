...
/*
 ...
 $Id: rcmail.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcmail
{
	...
	private function startup()
	{
		...
		else $GLOBALS['OUTPUT'] = $this->load_gui(!empty($_REQUEST['_framed']));
	...
	private function language_prop($lang)
	{
		...
		elseif (!isset($rcube_languages[$lang])) {
	...
	function login($username, $pass, $host = null)
	{
		...
		if (is_array($config['default_host'])) {
			...
			if (!$allowed)
			return false;
		} elseif (!empty($config['default_host']) && $host != rcube_parse_host($config['default_host']))
		return false;
		...
		if ($a_host['host']) {
			...
			elseif ($imap_ssl && $imap_ssl != 'tls' && (!$config['default_port'] || $config['default_port'] == 143))
			$imap_port = 993;
		}
		...
		*/
		...
		if (!empty($config['username_domain']) && strpos($username, '@') === false) {
			...
			elseif (is_string($config['username_domain']))
		...
		elseif ($config['auto_create_user']) {
	...
	public function autoselect_host()
	{
		...
		} else $host = rcube_parse_host($default_host);
	...
	public function gettext($attrib, $domain = null)
	{
		...
		;
		// text does not exist
		elseif (!($text_item = $this->texts[$name])) {
			return "[$name]";
		}
