...
/*
 ...
 $Id: rcube_imap.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcube_imap
{
	...
	function set_search_set($str = null, $msgs = null, $charset = null, $sort_field = null, $threads = false)
	{
		...
		if ($msgs === false)
			$msgs = array();
		elseif ($msgs != null && !is_array($msgs))
			$msgs = explode(',', $msgs);
	...
	function set_threading($enable = false)
	{
		...
		if ($enable) {
			if ($this->get_capability('THREAD=REFS'))
				$this->threading = 'REFS';
			elseif ($this->get_capability('THREAD=REFERENCES'))
				$this->threading = 'REFERENCES';
			elseif ($this->get_capability('THREAD=ORDEREDSUBJECT'))
	...
	private function _messagecount($mailbox = '', $mode = 'ALL', $force = false, $status = true)
	{
		...
		if ($this->search_string && $mailbox == $this->mailbox && ($mode == 'ALL' || $mode == 'THREADS') && !$force) {
			...
			else return count((array) $this->search_set);
		}
	...
	private function _list_headers($mailbox = '', $page = null, $sort_field = null, $sort_order = null, $recursive = false, $slice = 0)
	{
		...
		if (!$this->sort_field) {
			...
			} elseif ($max = $this->conn->countMessages($mailbox)) {
				list($begin, $end) = $this->_get_message_range($max, $page);
				$msg_index = range($begin+1, $end);
			} else $msg_index = array();
		...
		// use SORT command
		elseif ($this->get_capability('SORT')) {
		...
		elseif ($a_index = $this->conn->fetchHeaderIndex($mailbox, "1:*", $this->sort_field, $this->skip_deleted)) {
			asort($a_index); // ASC
	...
	function message_index($mbox_name = '', $sort_field = null, $sort_order = null)
	{
		...
		if (!isset($this->cache[$key]) && $this->search_string
			...
			elseif ($this->get_capability('SORT')) {
				...
				else $this->cache[$key] = $this->search_set;
			} else {
		...
		if (!$this->sort_field) {
			...
			} elseif ($max = $this->_messagecount($mailbox)) {
				$a_index = range(1, $max);
			}
		...
		elseif ($this->get_capability('SORT')) {
			...
			$this->cache[$key] = $a_index;
		} elseif ($a_index = $this->conn->fetchHeaderIndex(
	...
	private function _search_index($mailbox, $criteria = 'ALL', $charset = null, $sort_field = null)
	{
		...
		} elseif ($sort_field && $this->get_capability('SORT')) {
	...
	function in_searchset($msgid)
	{
		if (!empty($this->search_string)) {
			...
			else return in_array("$msgid", (array) $this->search_set, true);
		} else return true;
	}
	...
	function &get_structure($uid, $structure_str = '')
	{
		...
		if (!empty($structure)) {
			...
			else $this->struct_charset = $this->_structure_charset($structure);
	...
	function &_structure_part($part, $count = 0, $parent = '', $mime_headers = null)
	{
			...
			*/
			...
			for ($i=0; $i<count($part); $i++) {
				...
				if (!is_array($part[$i][0])) {
					...
					} elseif (in_array('name', (array) $part[$i][2]) && (empty($part[$i][3]) || $part[$i][3]=='NIL')) {
						$mime_part_headers[] = $tmp_part_id;
					}
				}
			}
		...
		*/
		...
		elseif ($struct->mimetype == 'message/rfc822') $di += 3;
	...
	private function _set_part_filename(&$part, $headers = null)
	{
		...
		elseif (!empty($part->d_parameters['filename*0'])) {
			$i = 0;
		...
		} elseif (!empty($part->ctype_parameters['name*0'])) {
			$i = 0;
	...
	function set_flag($uids, $flag, $mbox_name = null, $skip_cache = false)
	{
		...
		else $result = $this->conn->flag($mailbox, $uids, $flag);
		...
		if ($result >= 0) {
			...
			if ($this->caching_enabled && !$skip_cache) {
				...
				else $this->remove_message_cache($cache_key, explode(',', $uids));
			}
			// update counters
			if ($flag=='SEEN')
	...
	function save_message($mbox_name, &$message, $headers = '', $is_file = false)
	{
		...
		if ($this->mailbox_exists($mbox_name, true)) {
			...
			} else $saved = $this->conn->append($mailbox, $message);
		}
	...
	function move_message($uids, $to_mbox, $from_mbox = '')
	{
		...
		// moving failed
		elseif ($config->get('delete_always', false) && $tbox == $config->get('trash_mbox')) {
	...
	private function _parse_uids($uids, $mailbox)
	{
		if ($uids === '*' || $uids === '1:*') {
			...
			// @TODO: skip fetchUIDs() and work with IDs instead of UIDs (?)
				...
				else $uids = $this->conn->fetchUIDs($mailbox, $this->search_set);
	...
	function delete_mailbox($mbox_name)
	{
		...
		if (is_array($mbox_name))
			$a_mboxes = $mbox_name;
		elseif (is_string($mbox_name) && strlen($mbox_name))
	...
	function create_default_folders()
	{
		...
		foreach ($this->default_folders as $folder) {
			...
			elseif (!$this->mailbox_exists($folder, true))
	...
	function mod_mailbox($mbox_name, $mode = 'in')
	{
		...
		if (!empty($this->root_dir)) {
			...
			elseif (!empty($mbox_name)) // $mode=='out'
	...
	function set_caching($set)
	{
		...
		else $this->caching_enabled = false;
	}
	...
	private function check_cache_status($mailbox, $cache_key)
	{
		...
		elseif (abs($msg_count - $cache_count) < $msg_count/10)
			return -1;
		else return -2;
	}
	...
	private function _sort_mailbox_list($a_folders)
	{
		...
		foreach ($a_folders as $i => $folder) {
			...
			if (($p = array_search($folder, $this->default_folders)) !== false && !$a_defaults[$p])
				$a_defaults[$p] = $folder;
			else $folders[$folder] = rcube_charset_convert($folder, 'UTF7-IMAP');
		}
