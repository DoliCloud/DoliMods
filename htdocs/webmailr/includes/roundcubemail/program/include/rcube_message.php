...
/*
 ...
 $Id: rcube_message.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcube_message
{
	...
	private function parse_structure($structure, $recursive = false)
	{
		...
		else $mimetype = $structure->mimetype;
		...
		elseif ($mimetype == 'multipart/alternative' && is_array($structure->parts)) {
			...
			foreach ($structure->parts as $p => $sub_part) {
				...
				if ($sub_mimetype == 'text/plain')
					$plain_part = $p;
				elseif ($sub_mimetype == 'text/html')
					$html_part = $p;
				elseif ($sub_mimetype == 'text/enriched')
					$enriched_part = $p;
				elseif (in_array($sub_mimetype, array('multipart/related', 'multipart/mixed', 'multipart/alternative')))
					$related_part = $p;
			}
			...
			} elseif ($enriched_part !== null) {
				$print_part = &$structure->parts[$enriched_part];
			} elseif ($plain_part !== null) {
			...
			elseif ($html_part !== null && empty($this->parts)) {
				$c = new stdClass;
				$c->type            = 'content';
		...
		elseif (is_array($structure->parts) && !empty($structure->parts)) {
			...
			for ($i=0; $i < count($structure->parts); $i++) {
				...
				} else $part_mimetype = $mail_part->mimetype;
				...
				elseif ($part_mimetype == 'application/ms-tnef') {
				...
				elseif (preg_match('/^(inline|attach)/', $mail_part->disposition) ||
					...
					elseif ($part_orig_mimetype == 'message/rfc822') {
						$this->parse_structure($mail_part, true);
					}
					// is a regular attachment
					elseif (preg_match('!^[a-z0-9-.+]+/[a-z0-9-.+]+$!i', $part_mimetype)) {
		...
		elseif ($structure->filename) {
			$this->attachments[] = $structure;
		}
		// message is a single part non-text (without filename)
		elseif (preg_match('/application\//i', $mimetype)) {
