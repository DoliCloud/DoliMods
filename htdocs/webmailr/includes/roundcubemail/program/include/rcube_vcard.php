...
/*
 ...
 $Id: rcube_vcard.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcube_vcard
{
	...
	public static function import($data)
	{
		...
		elseif (($charset = self::detect_encoding($data)) && $charset != RCMAIL_CHARSET) {
			...
			if (!preg_match('/^(BEGIN|END)$/i', $line[1]) && preg_match_all('/([^\\;]+);?/', $line[1], $regs2)) {
				...
				foreach ($regs2[1] as $attrid => $attr) {
					if ((list($key, $value) = explode('=', $attr)) && $value) {
						...
						} else $entry[strtolower($key)] = array_merge((array) $entry[strtolower($key)], (array) self::vcard_unquote($value, ','));
					} elseif ($attrid > 0) {
