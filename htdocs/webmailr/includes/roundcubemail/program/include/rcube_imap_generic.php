...
/*
 ...
 $Id: rcube_imap_generic.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcube_imap_generic
{
	...
	function connect($host, $user, $password, $options = null)
	{
		...
		else $this->fp = @fsockopen($host, $this->prefs['port'], $errno, $errstr);
		...
		if (!preg_match('/^\* (OK|PREAUTH)/i', $line)) {
			...
			else $this->error = sprintf("Empty startup greeting (%s:%d)", $host, $this->prefs['port']);
			$this->errornum = -2;
			return false;
		}
	...
	function select($mailbox)
	{
		...
		if ($this->putLine("sel1 SELECT \"".$this->escape($mailbox).'"')) {
			do {
				...
				} elseif (preg_match('/\[?PERMANENTFLAGS\s+\(([^\)]+)\)\]/U', $line, $match)) {
	...
	function fetchHeaderIndex($mailbox, $message_set, $index_field = '', $skip_deleted = true, $uidfetch = false)
	{
		...
		elseif ($mode == 2) {
			...
			else $request = " $cmd $message_set ($index_field$deleted)";
		} elseif ($mode == 3)
				...
				if (preg_match('/^\* [0-9]+ FETCH \((.*) BODY/s', $line, $matches)) {
					...
					if ($parts_count>=6) {
						for ($i=0; $i<$parts_count; $i=$i+2) {
							if ($a[$i] == 'UID')
								$result[$id]->uid = $a[$i+1];
							elseif ($a[$i] == 'RFC822.SIZE')
								$result[$id]->size = $a[$i+1];
							elseif ($a[$i] == 'INTERNALDATE')
								$time_str = $a[$i+1];
							elseif ($a[$i] == 'FLAGS')
								$flags_str = $a[$i+1];
						}
	...
	private function _listMailboxes($ref, $mailbox, $subscribed = false)
	{
		...
		if (is_array($folders)) {
			return $folders;
		} elseif ($this->parseResult($line) == 0) {
			return array();
		}
		...
		if ($line[$len-1] == ')' && $line[$len-2] != '(') {
			...
			while ($bytes > 0) {
				...
				if ($mode == 1) {
					...
					else $result .= base64_decode($line);
				} elseif ($mode == 2) {
					...
					else $result .= quoted_printable_decode($line);
				} elseif ($mode == 3) {
					$line = rtrim($line, "\t\r\n\0\x0B");
					...
					else $result .= convert_uudecode($line);
				} else {
					$line = rtrim($line, "\t\r\n\0\x0B");
					if ($file)
						fwrite($file, $line . "\n");
