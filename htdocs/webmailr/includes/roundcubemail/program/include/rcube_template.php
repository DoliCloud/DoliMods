...
/*
 ...
 $Id: rcube_template.php,v 1.1 2011/08/01 19:22:12 eldy Exp $
...
class rcube_template extends rcube_html_page
{
	...
	public function set_skin($skin)
	{
		...
		else $skin_path = $this->config['skin_path'] ? $this->config['skin_path'] : 'skins/default';
	...
	public function button($attrib)
	{
		...
		if ($attrib['command']) {
			...
			} elseif ($attrib['task'] && in_array($attrib['task'], rcmail::$main_tasks)) {
				$attrib['href'] = rcmail_url($attrib['command'], null, $attrib['task']);
			} elseif (in_array($attrib['command'], $a_static_commands)) {
				$attrib['href'] = rcmail_url($attrib['command']);
			} elseif ($attrib['command'] == 'permaurl' && !empty($this->env['permaurl'])) {
	...
	public function request_form($attrib, $content = '')
	{
		...
		else return $this->form_tag($attrib, $hidden->show() . $content);
	}
	...
	private function login_form($attrib)
	{
		...
		} elseif (is_array($default_host) && ($host = array_pop($default_host))) {
			$hide_host = true;
