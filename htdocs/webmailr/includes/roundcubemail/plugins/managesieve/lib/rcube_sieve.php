...
/*
  ...
  $Id: rcube_sieve.php,v 1.1 2011/08/01 19:22:56 eldy Exp $
...
class rcube_sieve_script
{
			...
			foreach ($rule['tests'] as $test) {
				...
				switch ($test['test']) {
					...
					case 'exists':
						...
						else $tests[$i] .= 'exists "' . $this->_escape_string($test['arg']) . '"';
					break;
					case 'header':
						$tests[$i] .= ($test['not'] ? 'not ' : '');
						...
						else $tests[$i] .= ' "' . $this->_escape_string($test['arg1']) . '"';
						if (is_array($test['arg2']))
						$tests[$i] .= ' ["' . implode('", "', $this->_escape_string($test['arg2'])) . '"]';
						else $tests[$i] .= ' "' . $this->_escape_string($test['arg2']) . '"';
					break;
				}
				$i++;
			}
	...
	private function _escape_string($content)
	{
		...
		if (is_array($content)) {
			...
			return $content;
		} else return preg_replace(array_keys($replace), array_values($replace), $content);
	}
