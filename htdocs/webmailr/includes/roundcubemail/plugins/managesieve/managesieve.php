 ...
 *
 * $Id: managesieve.php,v 1.1 2011/08/01 19:22:41 eldy Exp $
 */
...
class managesieve extends rcube_plugin
{
		...
		if ($action = get_input_value('_act', RCUBE_INPUT_GPC)) {
			...
			if ($action == 'up' && !$error) {
				if ($fid && isset($this->script[$fid]) && isset($this->script[$fid-1])) {
					...
					} else $this->rc->output->show_message('managesieve.filtersaveerror', 'error');
				}
				...
				else $filename = addcslashes($script_name, '\\"');
	...
	function managesieve_save()
	{
		...
		elseif (isset($_POST['_name'])) {
			...
			else foreach ($this->script as $idx => $rule)
			...
			foreach ($act_types as $idx => $type) {
				...
				switch ($type) {
					...
					case 'redirect_copy':
						...
						elseif (!check_email($this->form['actions'][$i]['target']))
					...
					case 'vacation':
						...
						if ($this->form['actions'][$i]['addresses']) {
							foreach ($this->form['actions'][$i]['addresses'] as $aidx => $address) {
								...
								elseif (!check_email($address)) {
									$this->errors['actions'][$i]['addresses'] = $this->gettext('noemailwarning');
									break;
								} else $this->form['actions'][$i]['addresses'][$aidx] = $address;
							}
						}
			...
			if (!$this->errors) {
				...
				if (!isset($this->script[$fid])) {
					...
					$new = true;
				} else $fid = $this->sieve->script->update_rule($fid, $this->form);
	...
	function filter_form($attrib)
	{
		...
		else $input_name = $input_name->show();
		...
		else $input_join = $input_join->show();
		...
		else $input_join = $input_join->show('anyof'); // default
