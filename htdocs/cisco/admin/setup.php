...
if ($actionsave)
{
    ...
    $i+=dolibarr_set_const($db,'CISCO_EXPORTKEY',trim(GETPOST('CISCO_EXPORTKEY','alpha')),'chaine',0,'',$conf->entity);
...
print_fiche_titre($langs->trans("CiscoSetup"),$linkback,'setup');
print '<br>';
...
//print $langs->trans("CiscoSetupOtherDesc")."<br>\n";
//print "<br>\n";
...
print '<td><input required="required" type="text" class="flat" id="CISCO_EXPORTKEY" name="CISCO_EXPORTKEY" value="' . (GETPOST('CISCO_EXPORTKEY','alpha')?GETPOST('CISCO_EXPORTKEY','alpha'):$conf->global->CISCO_EXPORTKEY) . '" size="40">';
...
$url=DOL_MAIN_URL_ROOT.(preg_replace('/'.preg_quote(DOL_URL_ROOT,'/').'/', '', $url)).'?key='.$conf->global->CISCO_EXPORTKEY;
