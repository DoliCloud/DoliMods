...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('alumni', 'survey', 'write');
	$permissiontodelete = $user->hasRight('alumni', 'survey', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("alumni")) {
	accessforbidden('Module alumni not enabled');
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Survey';
	$objectlabel = 'Survey';
	$uploaddir = $conf->alumni->dir_output;
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/alumni/survey_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/alumni/survey_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$trackid = 'alumnisurvey'.$object->id;
