...
/**
 *	\file       congespayesagenda.class.php
 *	\ingroup    congespayes/class
 *	\brief      Fichier d'agenda pour le module Congés Payés
 *	\version    $Id: congespayesagenda.class.php,v 1.00 2011/09/15 11:00:00 ahebert Exp $
 */
...
class CongesPayesAgenda extends CommonAgenda
{
	var $db;
	 ...
	 */
	function CommonTimetracking($DB)
	{
		$this->db = $DB;
	}
	...
	function getEvents($start,$end,$options)
	{
      ...
      if($nbr){
         ...
         while($res = $db->fetch_object($qry)){
            ...
            $data[$i]['icon']			= "<img src='".DOL_URL_ROOT."/congespayes/img/conges.png' height=13 width=13 />";
...
<table class='border' width='100%'>
   <tr>
      ...
      <td>
         ...
         <input type='hidden' name='selectedTab' value='congespayesagenda' />
...
HTML;
	...
	function getLink($action = "")
	{
      ...
      $action = ($action == 'create') ? 'request' : '';
		$url = DOL_URL_ROOT."/congespayes/fiche.php";
