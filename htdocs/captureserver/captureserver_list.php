...
/**
 *   	\file       htdocs/captureserver/captureserver_list.php
 *		\ingroup    captureserver
 *		\brief      List page for captureserver
 */
...
// load captureserver libraries
require_once __DIR__ . '/class/captureserver.class.php';
...
//$help_url="EN:Module_CaptureServer|FR:Module_CaptureServer_FR|ES:Módulo_CaptureServer";
$help_url='';
$title = $langs->trans('ListOfCapturedEvents');
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/captureserver/captureserver_card.php', 1).'?id='.$id);
	exit;
}
...
if ($user->hasRight('captureserver', 'delete')) $arrayofmassactions['predelete']='<span class="fa fa-trash paddingrightonly"></span>'.$langs->trans("Delete");
...
$topicmail="SendCaptureServerRef";
$modelmail="captureserver";
$objecttmp=new CaptureServer($db);
...
foreach ($object->fields as $key => $val) {
	...
	if (! empty($arrayfields['t.'.$key]['checked'])) {
		...
		if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) print $form->selectarray('search_'.$key, $val['arrayofkeyval'], $search[$key], $val['notnull'], 0, 0, '', 0, 0, 0, '', 'maxwidth100', 1);
...
while ($i < min($num, $limit)) {
	...
	foreach ($object->fields as $key => $val) {
		...
		if (! empty($arrayfields['t.'.$key]['checked'])) {
			...
			elseif ($key == 'ip') print dol_print_ip($obj->$key);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	$genallowed=$user->rights->captureserver->read;
	$delallowed=$user->rights->captureserver->write;
