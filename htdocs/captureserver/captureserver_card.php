...
$permissionedit=$user->rights->captureserver->write; // Used by the include of actions_lineupdown.inc.php
$permissiontoadd=$user->rights->captureserver->write; // Used by the include of actions_addupdatedelete.inc.php
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	*/
	...
	if ($action != 'presend' && $action != 'editline') {
    	...
    	if (empty($reshook))
    	{
    		...
    		*/
    		...
    		if (! empty($user->rights->captureserver->delete) || (! empty($object->fields['status']) && $object->status == $object::STATUS_DRAFT && ! empty($user->rights->captureserver->write)))
    		{
	...
	if ($action != 'presend')
	{
	    ...
	    $filedir = $conf->captureserver->dir_output . '/' . $objref;
	    $urlsource = $_SERVER["PHP_SELF"] . "?id=" . $object->id;
	    $genallowed = $user->rights->captureserver->read;	// If you can read, you can build the PDF to read content
	    $delallowed = $user->rights->captureserver->create;	// If you can create/edit, you can remove a file on card
	    print $formfile->showdocuments('captureserver', $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->modelpdf, 1, 0, 0, 28, 0, '', '', '', $soc->default_lang);
		*/
