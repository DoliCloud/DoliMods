...
/**
 * \file    test/phpunit/HelloAssoFunctionalTest.php
 * \ingroup helloasso
 ...
 *
 * @package Testhelloasso
 */
class HelloAssoFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 101750; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modHelloAsso')->click();
		} else {
			// Disable the module
			$this->byHref('modHelloAsso')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/helloasso/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('helloasso/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/helloasso/admin/about.php');
		$this->authenticate();
		return $this->assertContains('helloasso/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('helloassowidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modHelloAsso_HelloAssoTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modHelloAsso_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
