...
/**
 * \file    discussion/admin/setup.php
 * \ingroup discussion
 * \brief   Discussion setup page.
 */
...
require_once '../lib/discussion.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "discussion@discussion"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
$hookmanager->initHooks(array('discussionsetup', 'globalsetup'));
...
$item = $formSetup->newItem('DISCUSSION_CACHE_NAME_PREFIX');
$item->fieldValue = 'DISCUSSION_CACHE_CHANNEL';
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('DISCUSSION_CACHE_SIZE');
$item->fieldValue = 50;
$item->cssClass = 'maxwidth50';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('DISCUSSION_CACHE_TTL');
$item->fieldValue = 2000;
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/discussion/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=discussion-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DISCUSSION_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DISCUSSION_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DiscussionSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-discussion page-admin');
...
$head = discussionAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "discussion@discussion");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DiscussionSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DISCUSSION_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DISCUSSION_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
