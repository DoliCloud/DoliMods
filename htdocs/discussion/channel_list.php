...
/**
 *   	\file       channel_list.php
 *		\ingroup    discussion
 *		\brief      List page for channel
 */
...
require_once __DIR__.'/class/channel.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('discussion', 'channel', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("discussion")) {
	accessforbidden('Module discussion not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Channel';
	$objectlabel = 'Channel';
	$uploaddir = $conf->discussion->dir_output;
...
$title = $langs->trans("Channels");
//$help_url = "EN:Module_Channel|FR:Module_Channel_FR|ES:Módulo_Channel";
$help_url = '';
$morejs = array();
$morecss = array();
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."element_contact as s ON s.element_id = t.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_contact as tc ON s.fk_c_type_contact = tc.rowid";
...
if (!$user->hasRight('discussion', 'channel', 'readall')) {
	$sql .= " AND ( t.private = 0 OR (t.private = 1 AND s.fk_socpeople = ".((int) $user->id)." AND tc.element = '".$object->element."'))";
}
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/discussion/channel_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-discussion page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/discussion/channel_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendChannelRef";
$modelmail = "channel";
$objecttmp = new Channel($db);
