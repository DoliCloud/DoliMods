...
/**
 *  \file       channel_note.php
 *  \ingroup    discussion
 *  \brief      Tab for notes on Channel
 */
...
if ($enablepermissioncheck) {
	...
	$permissionnote = $user->hasRight('discussion', 'channel', 'write') && $ischanneladmin; // Used by the include of actions_setnotes.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
	$permissionnote = 1;
...
$title = $langs->trans('Channel').' - '.$langs->trans("Notes");
//$title = $object->ref." - ".$langs->trans("Notes");
$help_url = '';
//$help_url='EN:Customers_Orders|FR:Commandes_Clients|ES:Pedidos de clientes';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-discussion page-card_notes');
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Channel"), -1, $object->picto);
