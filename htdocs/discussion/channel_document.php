...
/**
 *  \file       channel_document.php
 *  \ingroup    discussion
 *  \brief      Tab for documents linked to Channel
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->discussion->multidir_output[$object->entity ? $object->entity : $conf->entity]."/channel/".get_exdir(0, 0, 0, 1, $object);
}
...
// Header
// ------
$title = $langs->trans("Channel")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Channel"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='channel/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'channel/'.dol_sanitizeFileName($object->ref).'/';
