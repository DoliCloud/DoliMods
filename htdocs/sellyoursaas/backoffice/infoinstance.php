...
if (! empty($instanceoldid) || $action == 'create')
{
	...
	if ($user->rights->sellyoursaas->sellyoursaas->write)
	{
		if ($action == 'create')
		{
			...
			if (empty($phonetoshow)) $phonetoshow=getvalfromkey($db,'dolicloud_saasplex.customer.tel',$organizationtoshow);
			...
			// Date end of trial
			if (empty($date_registration)) $date_registration=getvalfromkey($db,'dolicloud_saasplex.customer.acquired_at',$organizationtoshow);
		...
		elseif ($action == 'edit' && ! empty($id))
		{
			...
			print '<form method="post" action="'.$_SERVER["PHP_SELF"].'?instanceoldid='.$instanceoldid.'" name="formsoc">';
			print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			print '<input type="hidden" name="instanceoldid" value="'.$instanceoldid.'">';
			print '<input type="hidden" name="instanceold" value="'.$instanceold.'">';
