...
/**
 *	\file       htdocs/multiinstance/actions_multiinstance.class.php
...
class ActionsMultiinstance
{
	...
	function select_entities($selected='', $htmlname='entity', $option='', $login=0)
	{
		...
		if (is_array($this->dao->entities))
		{
			foreach ($this->dao->entities as $entity)
			{
				...
				{
					if (! empty($user->login) && ! empty($conf->multiinstance->transverse_mode) && ! empty($user->entity) && $this->checkRight($user->id, $entity->id) < 0) continue;
	...
	function assign_values($action='view')
	{
		...
		$this->template_dir = dol_buildpath('/multiinstance/tpl/');
	...
	/**
	 * 	Set values of global conf for multiinstance
	 *
	...
	function getLoginPageOptions($parameters=false)
	{
		...
		$select_entity='';
		$lastuser='';
		$lastentity=(! empty($conf->multiinstance->force_entity)?$conf->multiinstance->force_entity:$entity);
	...
	private function getTopRightMenu()
	{
		...
		$text = img_object('', 'multiinstance@multiinstance','id="switchentity" class="entity linkobject"');
				...
				$( "#dialog-switchentity" ).dialog({
					...
					buttons: {
						\''.$langs->trans('Ok').'\': function() {
							choice=\'ok\';
							$.get( "'.dol_buildpath('/multiinstance/ajaxswitchentity.php',1).'", {
								action: \'switchentity\',
								entity: $( "#entity" ).val()
							},
							function(content) {
