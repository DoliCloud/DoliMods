 ...
 * Copyright (C) 2019 Emilianodfgfdgdfgfdgdfgdfhg gdf gdf gdfg Pappacena <pappacenafood@gmail.com>
 *
...
/**
 * 	\defgroup   prestashopget     Module PrestashopGet
 *  \brief      PrestashopGet module descriptor.
 *
 *  \file       htdocs/prestashopget/core/modules/modPrestashopGet.class.php
 *  \ingroup    prestashopget
 *  \brief      Description and activation file for module PrestashopGet
 */
include_once DOL_DOCUMENT_ROOT .'/core/modules/DolibarrModules.class.php';


/**
 *  Description and activation class for module PrestashopGet
 */
class modPrestashopGet extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModulePrestashopGetName' not found (PrestashopGet is name of module).
		...
		// Key used in llx_const table to save module status enabled/disabled (where PRESTASHOPGET is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array('/prestashopget/css/prestashopget.css.php'),	// Set this to relative path of css file if module has its own css file
	 		'js' => array('/prestashopget/js/prestashopget.js.php'),          // Set this to relative path of js file if module must load a js on all pages
		...
		// Example: this->dirs = array("/prestashopget/temp","/prestashopget/subdir");
		$this->dirs = array("/prestashopget/temp");

		// Config pages. Put here list of php page, stored into prestashopget/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@prestashopget");
		...
		if (! isset($conf->prestashopget) || ! isset($conf->prestashopget->enabled))
		{
			$conf->prestashopget=new stdClass();
			$conf->prestashopget->enabled=0;
		}
        ...
        $this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@prestashopget:$user->rights->prestashopget->read:/prestashopget/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
        // $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@prestashopget:$user->rights->othermodule->read:/prestashopget/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
        ...
        $this->dictionaries=array(
            ...
            'tabcond'=>array($conf->prestashopget->enabled,$conf->prestashopget->enabled,$conf->prestashopget->enabled)												// Condition to show each dictionary
        );
        */
		...
		$this->menu[$r++]=array('fk_menu'=>'',			                // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'leftmenu'=>'',
								'url'=>'/prestashopget/prestashopgetindex.php',
								'langs'=>'prestashopget@prestashopget',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
								'position'=>1000+$r,
								'enabled'=>'$conf->prestashopget->enabled',	// Define condition to show or hide menu entry. Use '$conf->prestashopget->enabled' if entry must be visible if module is enabled.
		...
		$this->menu[$r++]=array(	'fk_menu'=>'fk_mainmenu=prestashopget,fk_leftmenu=prestashopget',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'titre'=>'New MyObject',
								'mainmenu'=>'prestashopget',
								'leftmenu'=>'prestashopget_myobject_new',
								'url'=>'/prestashopget/myobject_page.php?action=create',
								'langs'=>'prestashopget@prestashopget',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
								'position'=>1000+$r,
								'enabled'=>'$conf->prestashopget->enabled',  // Define condition to show or hide menu entry. Use '$conf->prestashopget->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
								'perms'=>'1',			                // Use 'perms'=>'$user->rights->prestashopget->level1->level2' if you want your menu with a permission rules
		...
		$this->export_icon[$r]='myobject@prestashopget';
	...
	{
		$result=$this->_load_tables('/prestashopget/sql/');
		...
		//$result1=$extrafields->addExtraField('myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'prestashopget@prestashopget', '$conf->prestashopget->enabled');
		//$result2=$extrafields->addExtraField('myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'prestashopget@prestashopget', '$conf->prestashopget->enabled');
		//$result3=$extrafields->addExtraField('myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'prestashopget@prestashopget', '$conf->prestashopget->enabled');
		//$result4=$extrafields->addExtraField('myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1 '', 0, 0, '', '', 'prestashopget@prestashopget', '$conf->prestashopget->enabled');
		//$result5=$extrafields->addExtraField('myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'prestashopget@prestashopget', '$conf->prestashopget->enabled');
