...
/**
 *		\file       htdocs/theme/bootstrap/style.css.php
 *		\brief      File for CSS style sheet bootstrap
 */
...
$path='/bootstrap/core';    // This value may be used in future for external module to overwrite theme

// Define image path files
$fontlist='arial,tahoma,verdana,helvetica';    //$fontlist='Verdana,Helvetica,Arial,sans-serif';
$img_head=dol_buildpath($path.'/theme/bootstrap/img/grid-18px-masked.png',1);
$img_button=dol_buildpath($path.'/theme/bootstrap/img/button_bg.png',1);
...
<?php } ?>
background-image: url(<?php echo $path.'/theme/bootstrap/img/grid-18px-masked.png' ?>); background-repeat: repeat;
	color: #101010;
...
<?php if ($usecss3) { ?>
	...
	background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/nav-overlay3.png',1); ?>) 50% 0% repeat-x;
<?php } else { ?>
    background-image: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/tmenu2.jpg',1); ?>);
...
ul.tmenu {	/* t r b l */
    ...
    background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/nav-overlay3.png',1); ?>) 0 0 repeat-x !important;
}
...
div.tmenuleft
{
	...
	background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/menutab-r.png',1); ?>) 0 0 no-repeat;
	margin-top: -4px;
}
div.tmenucenter
{
...
foreach($mainmenuusedarray as $key => $val)
{
	...
	if (! $found)
	{
		$url=dol_buildpath($path.'/theme/bootstrap/img/menus/generic'.$generic.".png",1);
		$found=1;
...
<?php } else { ?>
	background-image: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/tmenu.jpg',1); ?>);
    background-position:top;
    background-repeat:repeat-x;
<?php } ?>
...
<?php } else { ?>
    background-image: url(<?php echo $path.'/theme/'.$conf->theme.'/img/tmenu2.jpg' ?>) !important;
...
<?php } else { ?>
	background: #dee7ec url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/tab_background.png',1); ?>) repeat-x;
<?php } ?>
    -moz-box-shadow: 4px 4px 4px #CCC;
    -webkit-box-shadow: 4px 4px 4px #CCC;
    box-shadow: 4px 4px 4px #CCC;
...
#undertopmenu {
	/*	background-image: url("<?php echo dol_buildpath($path.'/theme/bootstrap/img/gradient.gif',1) ?>"); */
	background-repeat: repeat-x;
	margin-top: 12px;
}
...
<?php } else { ?>
	background: #7699A9;
	background-image: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/liste_titre2.png',1); ?>);
<?php } ?>
    color: #<?php echo $colortextmain; ?>;
    font-family: <?php print $fontlist ?>;
    font-weight: normal;
...
.ui-autocomplete-loading { background: white url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/working.gif',1) ?>) right center no-repeat; }
.ui-autocomplete {
	       position:absolute;
	       width:auto;
	       font-size: 1.0em;
...
.editkey_textarea, .editkey_ckeditor, .editkey_string, .editkey_email, .editkey_numeric, .editkey_select, .editkey_autocomplete {
	background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/edit.png',1) ?>) right top no-repeat;
	cursor: pointer;
}

.editkey_datepicker {
	background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/calendar.png',1) ?>) right center no-repeat;
	cursor: pointer;
}
...
.ecmjqft LI.wait { font-weight:normal; background: url(<?php echo dol_buildpath($path.'/theme/bootstrap/img/working.gif',1); ?>) left top no-repeat; }
