...
Ce document décrit les spécifications techniques pour la mise en oeuvre de SellYourSaas - système de déploiement et vente automatisé en SaaS d'une application WAMP (Dolibarr ERP CRM, GLPI, ...).
...
SellYourSaas est un projet Open Source né suite au besoin de fournir en temps réel des instances d'application Web dédiées (comme Dolibarr ERP CRM)
à des utilisateurs, à des fins de tests immédiat, avec les impératifs suivants:

	* Capable de gérer un nombre très élevé d'utilisateurs.
	* Cout très très faibles (au moins 10x inférieurs aux solutions Cloud par containers).
	* Déploiement en temps réel.
	* Offrir un accès complet et privé à l'application proposée, avec accès complet sans bridage de fonctionnalités ou paramétrage.
	* Offrir un SaaS ouvert, avec accès SSH, SFTP et base de donnée direct possible aux utilisateurs. 

Suite au déploiement de la v1, la plateforme évolua pour ajouter d'autres objectifs :
	...
	* Une souplesse dans le mode d'abonnement (fréquence, tarifs, service complémentaires, ...)
	* Compatible (ou à minima adaptable) pour toute application Web.
	* Gestion d'un réseau de revendeur.
	* Disponible en OpenSource.
	
Financé par les sociétés Open Source [NLTechno https://www.nltechno.com] et [Teclib https://www.teclib.com], SellYourSaas v2 a atteint ces objectifs et est aujourd'hui en production chez plusieurs sociétés proposants des services Saas (DoliCloud, NovaFirstCloud, GLPI-Network, ...). Voici un synthèse de ces capacités:

    * Déploiment de n'importe quelle application WAMP. Gestion de multiples solutions différents en même temps.
    * Gestion de différents niveaux de tarifications
    ...
    * Outils facilitant la maintenance et le support des clients.
    * Gestion d'un réseau de revendeur.
    * Coût d'infrastructure par instance < 20 centimes (Coût constaté sur le service commercial DoliCloud avec Dolibarr ERP CRM).
    * ... 
...
La première étape est de mettre à disposition un (ou plusieurs) serveur. Si on part sur plusieurs, serveur, l'un sera serveur *Master* et les autres seront *Serveurs de déploiement*. Le serveur *Master* pouvant aussi être *Serveur de déploiement*, il est possible de démarrer avec un seul serveur. 
 
=== Choix de l'OS: OS Ubuntu server

* On utilisera Ubuntu LTS minimal 16.04 ou 18.04.
...
Pour le ou les serveurs de déploiement: B2-15 ou + (soit server 2 core minimum - Mémoire 8 Go minimum - Coût en 2019: 22 euros / mois)
...
# Pour declarer une IP v6 (le masque est 128 chez OVH en ipv6) 
...
[source,bash]
---------------
exportfs -v -a			(if there is new entry to add)
exportfs -v -r			(if there is new entry to remove)
exportfs
...
Edition du fichier de config *myqlnd.cnf* dans */etc/mysql/mysql.conf.d* (si mysql) 
...
Sur le serveur master et sur chaque serveur de déploiement, donner l'accès localement au compte:
...
Donner les droits d'accès sur le master au compte de chaque serveur de déploiement:
...
30 3 * * * /home/admin/wwwroot/dolibarr_nltechno/htdocs/sellyoursaas/scripts/perms.ksh >/home/admin/logs/perms.log
...
- Créer un nouveau serveur OVH depuis le snapshot en spécifiant le nom du snapshot source et en indiquant le script suivant en *post installation*:
...
- Si on est parti sur un disque vierge, formater le nouveau disque dur de donnée. Pour cela, voir la chapitre <<ajout_de_disque>>.
  Si il s'agit d'un dique déjà formatté, TODO...
...
- Aller dans l'interface de gestion des serveurs OVH, pour modifier leur nom court. Ceci modifiera le fichier */etc/hostname* automatiquement (sinon modifier manuellement. Mettre un nom court, par exemple *monserveur*).

- Aller dans l'interface de gestion des IP OVH, pour ajouter aussi 
 
  - le reverse sur l'IP du nouveau serveur: monserveur.mondomain.com
  - le reverse sur l'IP virtuel si vous en avez pris une sur: with.mondomain.com
...
- Déclarer l'IP pour l'envoi d'emails:
  ...
  - Ajouter l'ip dans l'entrée SPF du DNS.

- Créer un compte sellyoursaas et permission au serveur de déploiement sur la base de donnée du master. Voir <<creer_un_compte_db_sellyoursaas>>.
...
- Effacer les fichiers dans :
  ...
  */etc/apache2/sellyoursaas-onffline*
...
- Renommer le fichier DNS */etc/bind/withX.mondomain.com.hosts* en */etc/bind/with(X+1).mondomain.com.hosts* et l'éditer pour prendre en compte le nouveau prefix.
...
- Vérifier que, depuis le serveur master, l'url *http://ipnouveauserverdeployment:8080/index.php/test* répond par un 404.
...
Voir documentation *afick.pl*

Exemple de contenu a ajouter au fichier */etc/afick.conf*
...
!/home/admin/.viminfo
!/home/admin/.mysql_history
!/home/ldestailleur/.bash_history
!/home/ldestailleur/.viminfo
!/home/ldestailleur/.mysql_history
!/root/.bash_history
!/root/.viminfo
!/root/.mysql_history
...
[source,bash]
---------------
/home/admin/wwwroot/dolibarr_nltechno/htdocs/sellyoursaas/scripts/clean.sh confirm
---------------
